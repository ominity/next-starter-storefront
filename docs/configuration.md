...
| `OMINITY_ACTIVE_CUSTOMER_COOKIE_NAME` | HttpOnly active customer preference cookie | `ominity_active_customer` |
| `OMINITY_ACTIVE_CUSTOMER_COOKIE_MAX_AGE_SECONDS` | Active customer cookie max age | `31536000` |
...
The authenticated `/channels/current` response is the source of truth for the current channel, available locales, and default language, country, and currency. The starter does not require a channel ID or locale environment variable.
...
| `OMINITY_TRACKING_ENABLED` | Enable browser tracking through `/api/events` in live API mode | `false` |
...
| `OMINITY_FEATURE_CUSTOMER_ACCOUNTS` | Enable switching, customer resources, team roles, permissions, and invitations; requires auth | `true` |
